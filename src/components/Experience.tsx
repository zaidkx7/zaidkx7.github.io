          ...
          {experiences.map((exp, index) => (
            <div key={index} className="relative mb-16 last:mb-0">
              <div className="flex flex-col md:flex-row items-start">
                {/* Timeline dot */}
                <div className="absolute left-0 md:left-1/2 transform md:translate-x-[-50%] w-6 h-6 rounded-full bg-card border-2 border-primary z-10"></div>
                
                {/* Content */}
                <div className={`w-full md:w-1/2 ${index % 2 === 0 ? 'md:pr-12 md:text-right' : 'md:pl-12 md:ml-auto'}`}>
                  <div className="bg-card/60 backdrop-blur-sm rounded-2xl p-6 border border-border shadow-xl relative overflow-hidden transition-colors">
                    {/* Decorative elements */}
